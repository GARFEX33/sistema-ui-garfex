...
- [x] En el cierre local, se confirmó el presupuesto por slices: las 1.598 líneas authored quedaron distribuidas en siete commits de implementación de hasta 400 líneas authored cada uno; `pnpm-lock.yaml` y `routeTree.gen.ts` permanecen como identidad generada. Se verificaron los hashes congelados conocidos, todos los comandos reportaron resultados reales y no existen ramas, PRs, remotos ni pushes no autorizados. <!-- sdd-owner: parent -->
- [x] El cambio conserva documentada como limitación la comparación visual interactiva lado a lado/overlay por ausencia de `xdg-open`; Storybook Chromium y Playwright/axe sí pasan. Backend/API, datos runtime, estados adicionales, catálogo, responsive/touch y desktop packaging permanecen explícitamente fuera de alcance. <!-- sdd-owner: parent -->
