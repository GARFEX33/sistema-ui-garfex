...
| 12 | Ready / unstarted | `… → PR 11 → 📍 PR 12` / PR 11 | Exact v1 schemas/parsers/adapters and evaluation lease; 300–390 A+D |
| 13 | Ready after PR 12 / unstarted | `… → PR 12 → 📍 PR 13` / PR 12 | `modoCaptura: SELECCION` attributes and authoritative reconciliation; 320–395 A+D |
| 14 | Ready after PR 13 / unstarted | `… → PR 13 → 📍 PR 14` / PR 13 | Authoritative review and `expectedCatalogFingerprint` create; 320–395 A+D |
...
**Accepted authority for PRs 12–15:** backend commit `23e9440c2b832edb8e557134018ea812979c6452`; generated reference `convex/_generated/api.d.ts`; sources `convex/catalogoAdmin/{atributos.ts,recursos.ts,resourceValidators.ts}`; and consumer reference `contract-tests/resource-admin-consumer.ts`. The four public operations are `api.catalogoAdmin.atributos.obtenerDefinicionAtributo`, `api.catalogoAdmin.atributos.listarValoresPermitidosAtributo`, `api.catalogoAdmin.recursos.evaluarCreacionDesdeSelecciones`, and `api.catalogoAdmin.recursos.crearRecursoDesdeSelecciones`. Parse the published DTOs and only their explicit application dispositions; ordinary Convex transport failures are not a typed application-return union. Do not infer from `crearRecurso`, `ResourceCreateInput`, `tipoDato`, or legacy option endpoints.

### PR 12 — Exact schemas/parsers/adapters and authoritative evaluation lease

**Depends on:** PR 11. **Start → finish:** pending-only frontend → exact validated transport parsing and evaluation adoption guarded by request token, hierarchy/Unit context, and draft revision. **Concrete frontend targets:** extend `src/features/resources-master/resourcesMaster.types.ts` and `resourcesMaster.api.ts` with the four v1 inputs/results, Zod schemas/parsers, `ResourceOperation` mappings, `ResourceTransport` adapter calls, and `ResourcesMasterApi` methods; extend `resourceCreation.model.ts` from its current null `authoritativeEvaluation`/`catalogFingerprint` lease; cover `tests/unit/resourcesMasterApi.test.ts` and `tests/unit/resourceCreation.model.test.ts`. **Budget:** 300–390 A+D. **Verify:** focused Vitest parser/evaluation tests plus `pnpm typecheck`. **Rollback:** remove only v1 schemas/parsers/adapters and evaluation integration and return to pending.

- [ ] **RED:** Write failing parser tests from exact fixtures for nullable definition, `modoCaptura: SELECCION | LIBRE`, paginated typed allowed values, `INCOMPLETE | VALID | INVALID`, issues, fingerprint, and malformed/unknown response rejection. <!-- sdd-owner: implementation -->
- [ ] **GREEN:** Implement schemas/parsers and transport adapters before React, then an evaluation lease that adopts only current token/context/revision responses and fails closed without inventing fields or endpoint behavior. <!-- sdd-owner: implementation -->
- [ ] **TRIANGULATE/REFACTOR:** Prove stale, out-of-order, malformed, and transport-rejected evaluator responses cannot enable review/create; run the focused command and record its exact result. <!-- sdd-owner: implementation -->

### PR 13 — `modoCaptura: SELECCION` attributes and authoritative reconciliation

**Depends on:** PR 12. **Start → finish:** exact evaluation facts available → one assignment-ID-keyed `Atributos · n de total` sequence offers only allowed typed selections and reconciles buckets from backend facts. **Concrete targets:** feature-local attribute stage, the v1 allowed-value adapter, `resourceCreation.selectionDraft.ts`, `ResourceCreationShell.tsx`, and corresponding unit/RTL tests. **Budget:** 320–395 A+D. **Verify:** focused attribute/reconciliation Vitest/RTL command plus `pnpm typecheck`. **Rollback:** remove this stage and return to pending/evaluation boundary; preserve pure buckets.
...
**Depends on:** PR 13. **Start → finish:** evaluated selection sequence → review and create are driven only by a current `VALID` evaluation and the published `CREATED | CATALOG_CHANGED | INCOMPLETE | INVALID` disposition union. **Concrete targets:** feature-local review/result components, v1 create adapter/parser, `ResourceCreationShell.tsx`, `CrearRecursoSurface.tsx`, and exact-contract unit/RTL tests. **Budget:** 320–395 A+D. **Verify:** focused review/create Vitest/RTL command plus `pnpm typecheck`. **Rollback:** remove review/create integration and return to the evaluated selection boundary without touching legacy `crearRecurso`.
