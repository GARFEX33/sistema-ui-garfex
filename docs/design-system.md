...
| `--color-warning-subtle` | `#fff8d6`                  |
| `--font-sans`            | `Inter, Arial, sans-serif` |

`--color-success` / `--color-success-subtle` were added for Resources Master's creation-success toast (Slice C2e) — the same green already established visually by `catalog-hierarchy`'s (pre-existing, hardcoded) `.catalog-success-toast`, now expressed as real tokens instead of inline hex so new consumers reuse them instead of hardcoding. U6A adds `--color-warning: #8a6800` as the warning foreground for attention states that are not errors, and `--color-warning-subtle: #fff8d6` as its low-emphasis background. The tokens are mapped in `src/styles.css`'s `@theme inline` block for `text-warning`, `bg-warning-subtle`, and related semantic utilities. The audit found no prior warning equivalent; `error` and `info` semantic tokens remain absent and are not added speculatively. Other token categories (spacing, radius, shadows, z-index, breakpoints, motion, component sizes) are not centralized yet; see `references/atomic-vocabulary.md` in the skill for the target list.
...
- **`Button`** (`src/shared/ui/Button.tsx`) — wraps react-aria-components' `Button`. Variants: `accent` (default — filled, used for the primary action: "Crear Clase", "Siguiente", the "Nueva Clase"/"Nuevo recurso" triggers), `outline` (transparent background, primary-colored border/text — used for "Cancelar"/"Volver"), and `quiet` (compact, borderless text action for segmented controls, row actions, filters, and pagination). All variants share disabled and focus behavior. No `size` prop yet — every standard usage is the same size; use `quiet` when the semantic action is low emphasis rather than introducing a second standard size. Renders a real `<kbd>` child styled via an arbitrary `[&_kbd]` selector, matching the trigger buttons' shortcut hint.
- **`Dialog` / `DialogHeading` / `DialogContent` / `DialogActions`** (`src/shared/ui/Dialog.tsx`) — wraps the `ModalOverlay`/`Modal`/`Dialog` react-aria-components frame with the approved backdrop/card/heading/footer chrome (630px desktop width, border-primary card, "Esc cerrar" hint). `width` defaults to 630 and remains the desktop width; narrow viewports add semantic outer padding and cap the Modal to its available width instead of applying the desktop offset. `height` is optional and unset by default (content sizes naturally, capped at viewport height with scrolling) — pass it explicitly only for a dialog whose height is pixel-locked by an approved OpenPencil snapshot (e.g. `NuevaClaseSurface` passes `height={440}`).
...
- **`PartidaEstadoBadge`** (`src/features/compras/PartidaEstadoBadge.tsx`) — a feature-local compact inline badge for the exact purchase-line `LinkStatus` union. It always renders Spanish text and `role="status"` with an `Estado: …` accessible name; it uses success, warning, neutral, or primary semantic utility contracts without icons, raw colors, or arbitrary values. It remains local because its domain contract is specific to Compras.
